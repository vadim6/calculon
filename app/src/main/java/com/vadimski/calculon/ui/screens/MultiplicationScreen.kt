...
import com.vadimski.calculon.ui.components.MultiplicationProblemView
...
import com.vadimski.calculon.viewmodel.MultiplicationViewModel

@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun MultiplicationScreen(onBack: () -> Unit) {
    val vm: MultiplicationViewModel = viewModel()
    val state by vm.state.collectAsState()

    var input by remember(state.currentStepIndex, state.isComplete, state.totalProblems, state.feedback) { mutableStateOf("") }

    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text("Multiplicación — Nivel ${state.level}") },
                navigationIcon = {
        ...
        Column(
            ...
            if (!state.isComplete) {
                ...
                // Prompt: "A × B = ?" with carry hint if carryIn > 0
                val prompt = buildString {
                    append("${step.multiplicandDigit} × ${step.multiplierDigit} = ?")
                    if (step.carryIn > 0) append("  (llevas ${step.carryIn})")
                }
                Text(
                    text = prompt,
                ...
                when (state.feedback) {
                    ...
                    StepFeedback.CORRECT -> {
                        ...
                        if (isLastStep) {
                            // Peek-ahead: current step has no error (user just got it right)
                            val perfect = !state.currentStepHadError && state.completedResults.none { it.hadError }
                            Spacer(Modifier.height(4.dp))
                            Text(
                                text = if (perfect) "¡Perfecto! Sin errores" else "Completado con errores",
                                color = if (perfect) SuccessGreen else MaterialTheme.colorScheme.onSurface,
                                fontWeight = FontWeight.SemiBold,
                                fontSize = 16.sp
                            )
                            Spacer(Modifier.height(6.dp))
                            Button(
                                onClick = { vm.finishAndNext() },
                                modifier = Modifier.fillMaxWidth(),
                    ...
                    StepFeedback.SHOW_SOLUTION -> {
                        Text(
                            "La respuesta es ${step.rawProduct}",
                ...
                if (state.feedback == StepFeedback.NONE || state.feedback == StepFeedback.WRONG) {
                    NumberPad(
                        onDigit = { d -> if (input.length < 2) input += d },
